...
export const createProject = async (req, res) => {
    try{
       ...
       const {workspaceId, name, description, status, start_date, end_date, teamMembers, teamlead, progress, priority} = req.body;
...
export const updateProject = async (req, res) => {
    try{
       const {userId} = await req.auth();
       const {id, workspaceId, name, description, status, start_date, end_date, teamMembers, teamlead, progress, priority} = req.body;
    ...
    try{
       ...
       const {email} = req.body;
        ...
        if(project.team_lead !== userId){
            return res.status(403).json({ message: "Only project team leads can add members" })
        }
